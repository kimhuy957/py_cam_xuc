...
output_file = file_path.split('\\')[-1].replace('.csv', '_ket_qua1.csv')
df['predicted_sentiment'] = model.predict(tfidf.transform(df['processed_text']))
df.to_csv(output_file, index=False)
logging.info(f"Kết quả được lưu vào: {output_file}")
print(f"Kết quả được lưu vào: {output_file}")

# Lưu mô hình Naive Bayes và vectorizer (TF-IDF) để sử dụng lại sau này
